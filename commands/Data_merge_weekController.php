...
use app\modules\appcake\models\DataSummaryWeek;
use app\components\Common;
use Yii;
class Data_merge_weekController extends Controller
{
    /**
     * 汇总数据合并到data_summary_week
     */
    public function actionAdData($strartTime=null,$endTime=null)
    {
        if($strartTime == null){
            $strartTime = isset($_SERVER["argv"][2]) ? $_SERVER["argv"][2] : date('Y-m-d',strtotime('-7 day'));
            $endTime = isset($_SERVER["argv"][3]) ? $_SERVER["argv"][3] : date('Y-m-d',time());
        }
        $mergeModel = new DataSummaryWeek();
        $isInserted = $mergeModel->IsInserted($endTime);
        if($isInserted){
            $logStr = date("Y-m-d H:i:s",time()) ." 查询以{$endTime}开始往前7天数据认定已经执行过,停止任务.";
            echo $logStr;
        ...
        $sql_1 = "INSERT INTO `data_summary_week` (date,app_id,name,camp_id,country,countries,category,ad_source,click,download,install,cake_active,h_click,h_active,analog_click,CP,AP,TA,TP,payout_amount,income) VALUES ";
        $i = 0;
        $sql = '';
        $times = 0;
        ...
        foreach($pKey as $k => $v)
        {
            ...
            $sql .=  $sql_1 . "('{$date}',$app_id,'{$name}','{$camp_id}','{$country}','{$countries}','{$category}','{$ad_source}',$click,$down,$install,$active,$hookClick,$hookActive,$analogClick,$cp,$ap,$ta,$tp,$payout_amount,$income);";
    ...
    public function writeClickData($strartTime,$endTime){
        ...
        $sql = ''; $num = 0;
        $sql_1 = "INSERT INTO `data_summary_week` (date,app_id,country,click) VALUES ";
    ...
    public function writeDownData($strartTime,$endTime){
        ...
        $times = ceil($count / $this->limit);
        $sql = ''; $num = 0;
        $sql_1 = "INSERT INTO `data_summary_week` (date,app_id,country,download) VALUES ";
    ...
    public function writeInstallData($strartTime,$endTime){
        ...
        $times = ceil($count / $this->limit);
        $sql = ''; $num = 0;
        $sql_1 = "INSERT INTO `data_summary_week` (date,app_id,country,install) VALUES ";
    ...
    public function writeAppInfo($date,$isAd = true){
        ...
        $times = ceil($count / $this->limit);
        $sql = ''; $num = 0;
        $sql_1 = "update `data_summary_week` set  ";
